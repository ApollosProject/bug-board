...
class GitHubOAuthTest(unittest.TestCase):
    ...
    def test_logout_requires_post_and_clears_the_authenticated_session(self):
        ...
        self.assertIn('id="logout-dialog"', page)
        self.assertIn("Are you sure you want to log out of Bug Board?", page)
        self.assertIn('class="close"', page)
        self.assertNotIn('rel="prev"', page)
        self.assertIn("typeof logoutDialog?.showModal !== 'function'", page)
        self.assertIn("event.preventDefault()", page)
        self.assertIn('form="logout-form"', page)
        self.assertIn("logoutDialog?.close?.()", page)
