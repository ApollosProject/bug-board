...
def get_open_issues_for_person_all_teams(login: str):
    """Return open issues assigned to a given Linear username across all teams."""
    query = gql(
        """
        query OpenIssuesAllTeams($login: String!, $cursor: String) {
          issues(
            first: 50
            after: $cursor
            filter: {
    ...
    issues = []
    while True:
        data = _execute(query, variable_values={"login": login, "cursor": cursor})
        issues += data["issues"]["nodes"]
        if not data["issues"]["pageInfo"]["hasNextPage"]:
            break
        cursor = data["issues"]["pageInfo"]["endCursor"]
...
def get_completed_issues_for_person_all_teams(login: str, days=30):
    """Return completed issues for a user over the last `days` days across all teams."""
    query = gql(
        """
